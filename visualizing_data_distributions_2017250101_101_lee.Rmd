---
title: "visualizing_data_distributions_2017250101_101_lee"
...
```{r}
female_percentiles <- sort(female) %>% .[c(10, 30, 50, 70, 90)]
male_percentiles <- sort(male) %>% .[c(10, 30, 50, 70, 90)]
p <- data.frame("female_percentiles" = female_percentiles, "male_percentiles" = male_percentiles)
```
...
normal_app <- pnorm(81, mean = m, sd = s) - pnorm(79, mean = m, sd = s)

exact_cal / normal_app
```
...
```{r}
pnorm(84, mean = 69, sd = 3, lower.tail=FALSE)
```
...
```{r}
10 / (1000000000 * (1 - pnorm(84, mean = 69, sd = 3)))
```
...
```{r}
150 / (1000000000 * (1 - pnorm(80, mean = 69, sd = 3)))
```
