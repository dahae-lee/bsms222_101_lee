---
title: "rbasic_2_2017250101_101_lee"
...
11. class of class(a<-1) is numeric, not integer. R defaults to numeric and to force an integer, you need to add the letter L. Confirm that the class of 1L is integer.
...
```{r}
ranks <- (city_temps$temp)
my_df <- data.frame(name = city_temps$name, ranks = rank(city_temps$temp))
```
...
my_df <- data.frame(names = murders$state, ranks = murders$population[ind])
```
...
5. n a previous exercise we computed the murder rate for each state and the average of these numbers. How many states are below the average?
...
```{r}
state_populations <- murders$population
