---
title: "data_visualization_principles_2017250101_101_lee"
...
murders %>% mutate(rate = total/population*100000) %>%
  ...
  mutate(region = factor(region)) %>%
  mutate(region = reorder(region, median_rate)) %>%
  ggplot(aes(region, median_rate)) +
    geom_bar(stat = "identity")
...
```{r}
sm <- us_contagious_diseases %>% 
  filter(disease == "Smallpox" & weeks_reporting >= 10 & !is.na(population)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting)

sm %>% mutate(state = reorder(state, rate)) %>%
  ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip() 
```
...
```{r}
us_contagious_diseases %>% mutate( !is.na(population)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
...
```{r}
cal <- us_contagious_diseases %>% filter(state == 'California') %>%
  mutate( weeks_reporting >= 10 & !is.na(population)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting)
...
```{r}
us_contagious_diseases %>% group_by(state, year, disease) %>%
  mutate(!is.na(population)) %>%
  summarize(rate = sum(count) / sum(population) * 10000 * 52 / sum(weeks_reporting)) %>%
  mutate(!is.na(rate)) %>%
...
```{r}
us_contagious_diseases %>% filter(disease == "Smallpox") %>%
  mutate( !is.na(population)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  ggplot(aes(year, rate)) +
    geom_line()
...
us_contagious_diseases %>% group_by(state, year, disease) %>%
  mutate(!is.na(population)) %>%
  summarize(rate = sum(count) / sum(population)) %>%
  mutate(!is.na(rate))
